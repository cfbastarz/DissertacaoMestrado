...
%As séries temporais e os campos médios de precipitação são apresentados abaixo (\autoref{seriesprecs} e \autoref{camposprecs}) e são referentes ao mês de Janeiro de 2003, para os experimentos SAP e CAP e os conjuntos de dados de precipitação do SALLJEX e GPCP. 
...
%A série de precipitação dos experimentos SAP e CAP (\autoref{seriesprecs}) representa os acumulados diários de precipitação das 06Z de cada dia. Para as observações de precipitação do SALLJEX, a série representa os acumulados diários de precipitação das 12Z de cada dia e, a série do GPCP representa os acumulados diários de precipitação às 00Z de cada dia.

%A figura abaixo (\autoref{camposprecs}) apresenta as médias dos acumulados de precipitação dos experimentos SAP e CAP e dos projetos SALLJEX e GPCP.

%\begin{figure}[!h]
%\centering
%\includegraphics[height=6cm]{./figs/campos_precs.png}
%\caption{Médias espaciais dos acumulados de precipitação para o mês de Janeiro de 2003 para os experimentos SAP e CAP e para os projetos SALLJEX e GPCP. Umidades estão em $mm/mês$.}
%\label{camposprecs}
%\end{figure}

%\break

%Na figura \autoref{seriesprecs} (quadro AS), nota-se que as séries de precipitação do SALJJEX e GPCP apresentam valores altos de precipitação em alguns dias do mês (entre os dias 4 e 7; 11 e 13; 17 e 21 e entre 22 e 25). Os dados do SALLJEX e GPCP possuem alta resolução espacial e por isso conseguem representar os acumulados diários com maior refinamento. as séries dos experimentos SAP e CAP, no entanto, apresentam séries de precipitação menos intensas, o que pode ser devido ao fato de que as previsões de precipitação produzidas pelos experimentos, não incorpore a grande quantidade de observações que podem ser encontradas nos projetos SALLJEX e GPCP. Apesar disto e, levando-se em consideração que o experimento CAP assimila a precipitação do TRMM, pode-se perceber que na região da Bacia do Prata (quadro BP - em que a média espacial das séries foi calculada nesta região), esse experimento consegue reproduzir melhor os acumulados diários de precipitação. O mesmo pode ser notado nas médias dos acumulados de precipitação (\autoref{camposprecs}). Nesta figura, percebe-se que o experimento CAP apresenta um campo precipitação mais compatível com os campos dos projetos SALLJEX e GPCP. 
...
As figuras a seguir (\autoref{fig30a}, \autoref{fig30b}, \autoref{fig31a}, \autoref{fig31b}, \autoref{fig32a}, \autoref{fig32b}, \autoref{fig33a}, \autoref{fig33b}, \autoref{fig34a}, \autoref{fig33b}) mostram as séries temporais do Viés e do Erro Quadrático Médio das análises em relação à reanálise do NCEP, para os níveis de 850, 500 e 250 $hPa$ para as seguintes variáveis: Altura Geopotencial [$mgp$], Temperatura do Ar [$ºC$], Ventos Zonal e Meridional [$ms^{1}$] e Umidade Relativa [$\%$], durante 02 e 30 de Janeiro de 2003.
